...
export function useMangroveBook({
  ...
  contextMarket?: MarketParams | null
}) {
  const mangrove = useMangroveAddresses()
  const { defaultChain } = useDefaultChain()
  const { currentMarket } = useMarket()
  const market = contextMarket ? contextMarket : currentMarket
...
async function getPoolBook(
  ...
  try {
    ...
    console.log("market getBpoolBook", market)
    const nOffers = depth
...
export function usePoolBook({
  ...
  const { currentMarket } = useMarket()
  const market = contextMarket ? contextMarket : currentMarket
  const { data: mangroveBook } = useMangroveBook({ contextMarket: market })
  ...
  return useQuery({
    queryKey: [
      ...
      contextMarket?.base.address,
      contextMarket?.quote.address,
      client?.key,
    ],
    queryFn: async () => {
      console.log(1)
      if (!client.key || !pool || !market || !mangroveBook) return null
      console.log(2)
      const quoteBalance =
